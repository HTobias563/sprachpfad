...
export default {
  ...
  actions: {
    ...
    show(ctx, el, ev) {
      ...
      host.innerHTML = String(h`<button class="x" data-act="close" aria-label="Schließen">${raw(icon('close'))}</button><div class="t" lang="${ctx.lang.code}">${it.text}</div><div class="d">${it.de}</div>${it.hint ? h`<div class="hnt">Klingt wie: ${it.hint}</div>` : ''}<button class="spk" data-act="say" aria-label="Anhören">${raw(icon('speaker'))}</button>`);
